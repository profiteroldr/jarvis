...
# Metinden -> Konuşmaya Çevirme Mekanizması (Yapay Zeka Seslendirme)
def speak(string):
...
# Komut Verme (Konuşmadan -> Metine - Sesli Asistan Temel Algılama ve Kontrolü)
def record():
...
# Mekanizma Uyandırma (Jarvis Başlatma)
def jarvis():
    # Döngü 2
    b=1
    while b: # b=1 olduğu sürece çalışacaktır.
        girdi_1 = record() # Kullanıcıdan ses alıyoruz.
        if girdi_1 == "Leyla": # Kullanıcı eğer "Jarvis" derse b=0 olacak, "Döngü 2" kırılacak sonraki "Döngü 1" içindeki fonksiyon olan speak() çalışacak.
            b=0
...
run() # Mekanizma'nın çalıştırılmasını sağlıyor.
